...
Describe "network.ps1 — Мережева безпека (25H2 Baseline)" {

    Context "NetBIOS — вимкнути через GPO (25H2 Baseline)" {
        BeforeAll {
            $item = $settings | Where-Object { $_.Name -match "NetBIOS.*GPO.*25H2" }
            $item.Apply.Invoke()
        }
        It "ConfigureNBT дорівнює 2" {
            Get-ItemPropertyValue `
                "HKLM:\SOFTWARE\Policies\Microsoft\Windows NT\DNSClient" `
                "ConfigureNBT" | Should -Be 2
        }
