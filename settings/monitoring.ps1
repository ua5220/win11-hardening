<#
.SYNOPSIS
    Моніторинг безпеки: виявлення вторгнень, аудит маркерів, кешування, OPSEC
.NOTES
    ...
    - Підміна часового поясу (OPSEC / Timezone Spoofer)
#>
...
- Підозрілі командлети (Invoke-Mimikatz, Add-MpPreference -ExclusionPath тощо)
...
"@
    Apply = {
        $suspiciousPatterns = @(
            ...
            'Set-MpPreference.*Disable', 'sekurlsa', 'kerberos::',
            'token::elevate', 'FromBase64String', 'Reflection\.Assembly'
        )
...
public class TokenAudit {
    ...
    public static extern bool OpenProcessToken(IntPtr hProcess, uint access, out IntPtr token);
    [DllImport("advapi32.dll", SetLastError=true)]
    public static extern bool GetTokenInformation(IntPtr token, int infoClass, IntPtr buf, uint bufLen, out uint retLen);
...
'@ -ErrorAction SilentlyContinue
        ...
        Get-Process -ErrorAction SilentlyContinue | ForEach-Object {
            if ($knownSafe -contains $_.Name.ToLower()) { return }
            try {
                $privs = whoami /priv 2>$null  # Для поточного процесу
                # Альтернатива: перевірити через Get-ProcessToken або WMI
                $wmi = Get-WmiObject Win32_Process -Filter "ProcessId=$($_.Id)" -ErrorAction SilentlyContinue
                if ($wmi) {
                    # Перевірка чи процес запущений від SYSTEM або з підвищеними правами
                    $owner = $wmi.GetOwner()
                    if ($owner.User -eq 'SYSTEM' -and $_.Name -notmatch 'System|Idle|Registry') {
        ...
        # Пряма перевірка через whoami для поточного контексту
        $currentPrivs = whoami /priv 2>$null | Out-String
        if ($currentPrivs -match 'SeImpersonatePrivilege') {
...
# ════════════════════════════════════════════════════════════════════════
# ── РОЗДІЛ 66: TIMEZONE SPOOFER (OPSEC) ─────────────────────────────────
# ════════════════════════════════════════════════════════════════════════

[PSCustomObject]@{
    Group = "OPSEC / Маскування"
    Name  = "Timezone Spoofer — підміна часового поясу (OPSEC)"
    Desc  = @"
Змінює часовий пояс системи для ускладнення кореляції часу в логах:
  Поточний → UTC (або інший заданий пояс)
Корисно при проведенні тестів на проникнення або для ускладнення
форензичного аналізу на основі часових міток.
УВАГА: змінює системний час — може вплинути на Kerberos (±5 хв),
SSL-сертифікати, заплановані задачі та часові мітки файлів.
Оригінальний часовий пояс зберігається в реєстрі для відновлення.
...
"@
    Apply = {
        ...
        try {
            ...
            Write-AppLog -Level 'WARN' -Message "OPSEC: Оригінальний пояс '$currentTZ' збережено в реєстрі для відновлення."
        } catch {
